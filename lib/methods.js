Meteor.methods({
  ...
  updatePicture(data) {
    ...
    check(data, String);
    return Meteor.users.update(this.userId, { $set: { 'profile.picture': data.toString() } });
  },
  ...
  updateSlider(data) {
    ...
    check(data[0], String); // id
    check(data[1], Number); // value

    return Sliders.update({_id: data[0]}, { $set: { 'value': data[1], 'username': Meteor.user().profile.name, 'timestamp': Date.now()}});
  }
});